...
function getCookieConfig(isProduction) {
  return {
    ...
    path: "/",
    domain: isProduction ? ".pourdian.com" : undefined, // برای دسترسی از همه ساب‌دامین‌های pourdian.com
  ...
  async registerWithEmail(req, res) {
    try {
      ...
      res.cookie("token", token, {
        ...
        path: "/",
        domain: isProduction ? ".pourdian.com" : undefined,
  ...
  async registerWithMobile(req, res) {
    try {
      ...
      res.cookie("token", token, {
        ...
        path: "/",
        domain: isProduction ? ".pourdian.com" : undefined, // در لوکال `undefined` باشد
        sameSite: isProduction ? "None" : "Lax", // در سرور `None`، در لوکال `Lax`
      });
